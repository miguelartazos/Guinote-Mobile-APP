...
const getCardStrength = (card: Card, trumpSuit: string, leadSuit?: string): number => {
  const baseValue = getCardPoints(card.rank);
  
  if (card.suit === trumpSuit) {
    return 1000 + baseValue; // Trump always wins
  } else if (card.suit === leadSuit) {
    return 100 + baseValue; // Must follow suit
  } else {
    return baseValue; // Can't win trick
  }
};
...
const canCambiar7 = (hand: Card[], trumpSuit: string, trumpCard: Card, isVueltas: boolean): boolean => {
  ...
  if (trumpCards.length > 0) {
    if (currentWinningCard.card.suit === trumpSuit) {
      ...
      const higherTrumps = trumpCards.filter(c =>
        getCardStrength(c, trumpSuit) > getCardStrength(currentWinningCard.card, trumpSuit)
      );
...
const selectMediumCard = (
  ...
  const sortedCards = [...validCards].sort((a, b) => 
    getCardPoints(b.rank) - getCardPoints(a.rank)
  );
...
const selectHardCard = (
  ...
  return selectMediumCard(validCards, gameState.current_trick, gameState.trump_suit, adjustedPersonality);
};
...
serve(async (req) => {
  ...
  try {
    ...
    const { gameStateId, playerIndex, difficulty = 'medium', personality = 'balanced' } = await req.json();
