-- ============================================
-- CLEAN FIX FOR "JUGAR CON AMIGOS" 
...
DROP FUNCTION IF EXISTS toggle_ready(UUID);
DROP FUNCTION IF EXISTS start_game(UUID);
DROP FUNCTION IF EXISTS add_ai_player(UUID, TEXT, TEXT);
DROP FUNCTION IF EXISTS get_online_friends(UUID);

-- Keep these as they might have the right signature already
-- DROP FUNCTION IF EXISTS ensure_user_exists();
-- DROP FUNCTION IF EXISTS create_room(TEXT, BOOLEAN);

-- STEP 2: Ensure auth trigger exists for user creation
...
-- STEP 3: ensure_user_exists (keep if exists, update if needed)
...
-- STEP 5: create_room (update if exists)
...
-- STEP 6: join_room (NEW)
CREATE FUNCTION join_room(p_room_code TEXT)
RETURNS JSON
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
-- STEP 7: leave_room (NEW with VOID return)
CREATE FUNCTION leave_room(p_room_id UUID)
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
-- STEP 8: toggle_ready (NEW)
CREATE FUNCTION toggle_ready(p_room_id UUID)
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
-- STEP 9: start_game (NEW)
CREATE FUNCTION start_game(p_room_id UUID)
RETURNS JSON
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
-- STEP 10: add_ai_player (NEW)
CREATE FUNCTION add_ai_player(
  p_room_id UUID,
...
-- STEP 11: get_online_friends (NEW)
CREATE FUNCTION get_online_friends(p_user_id UUID)
RETURNS TABLE(
  friend_id UUID,
  username TEXT,
  display_name TEXT,
...
-- STEP 13: Enable Row Level Security (if not already enabled)
ALTER TABLE users ENABLE ROW LEVEL SECURITY;
ALTER TABLE rooms ENABLE ROW LEVEL SECURITY;
ALTER TABLE room_players ENABLE ROW LEVEL SECURITY;
ALTER TABLE friendships ENABLE ROW LEVEL SECURITY;

-- STEP 14: Create RLS policies (if not exist)
DO $$
BEGIN
...
BEGIN
  ...
  RAISE NOTICE '2. Test with: node check-rpc-status.js';
  RAISE NOTICE '';
END
$$;